...
*   **🤖 Режим Агента (Agent Mode):**
    *   **Интегрированная песочница:** Безопасное выполнение Python-кода и Bash-команд в изолированных Docker-контейнерах (Python 3.12).
    ...
    *   **🎯 Система навыков (Skills):** RAG-система с embeddings для автоматического предоставления релевантного контекста из markdown-документов (7 навыков: core, ffmpeg, file-hosting, file-management, task-planning, video-processing, web-search).
    ...
    *   **🔗 Система хуков (Hooks):** Расширяемая архитектура для перехвата и кастомизации поведения агента на различных этапах выполнения (completion, registry).
    *   **🔄 Детектирование зацикливания:** Три уровня защиты от зацикливания агента (content_detector, tool_detector, llm_detector).
    ...
    *   **Долгая память и контекст:** Поддержка длинных цепочек рассуждений с автоматическим сжатием контекста (200K токенов).
...
*   **🔒 Безопасность:** Использование `unsafe_code = forbid`, строгие Clippy lints (`unwrap_used`, `too_many_lines`, `too_many_arguments` = forbid), отсутствие небезопасного кода.
...
*   **Tavily API:** Для веб-поиска в режиме Агента (опционально, feature flag `tavily`).
...
│   ├── mod.rs
│   ├── executor.rs            # главный исполнитель агента (~60KB)
│   ├── loop_detection/        # детектирование зацикливания
│   │   ├── mod.rs
│   │   ├── config.rs          # конфигурация детектирования
│   │   ├── content_detector.rs  # детектор повторяющегося контента
│   │   ├── tool_detector.rs   # детектор зацикливания инструментов
│   │   ├── llm_detector.rs    # LLM-анализ зацикливания
│   │   ├── service.rs         # сервис детектирования
│   │   └── types.rs
│   ├── skills/                # подсистема навыков (RAG/embeddings)
│   │   ├── mod.rs
│   │   ├── cache.rs           # кэширование навыков
│   │   ├── embeddings.rs      # генерация embeddings
│   │   ├── loader.rs          # загрузка markdown навыков
│   │   ├── matcher.rs         # семантическое сопоставление
│   │   ├── registry.rs        # реестр навыков
│   │   └── types.rs
│   ├── hooks/                 # хуки выполнения
│   │   ├── mod.rs
│   │   ├── completion.rs      # хук завершения задачи
│   │   ├── registry.rs        # реестр хуков
│   │   └── types.rs
│   ├── providers/             # провайдеры инструментов
│   │   ├── mod.rs
│   │   ├── filehoster.rs      # публичный хостинг файлов
│   │   ├── path.rs            # работа с путями
│   │   ├── sandbox.rs         # Docker песочница (~20KB)
│   │   ├── tavily.rs          # веб-поиск через Tavily
│   │   ├── todos.rs           # управление задачами
│   │   └── ytdlp.rs           # скачивание видео (~33KB)
│   ├── session.rs             # управление сессиями
│   ├── memory.rs              # долгосрочная память
│   ├── preprocessor.rs        # предобработка сообщений
│   ├── progress.rs            # отображение прогресса
...
│   ├── state.rs               # состояние бота
│   └── unauthorized_cache.rs  # кэш неавторизованных пользователей
├── llm/                       # интеграции с провайдерами LLM
│   ├── mod.rs
│   ├── common.rs              # общие структуры
│   ├── providers.rs           # реализация провайдеров
│   ├── openai_compat.rs       # OpenAI-совместимые провайдеры
│   └── http_utils.rs          # HTTP утилиты
├── sandbox/                   # управление Docker-песочницей
│   ├── mod.rs
│   └── manager.rs             # менеджер контейнеров
├── storage.rs                 # работа с Cloudflare R2/S3
├── config.rs                  # конфигурация и модели
└── utils.rs                   # вспомогательные функции (~21KB)
