...
*   **🤖 Режим Агента (Agent Mode):**
    ...
    *   **Инструменты (Tools):** Чтение/запись файлов, выполнение команд, поиск в интернете.
    *   **📋 Управление задачами (Todos):** Система `write_todos` для планирования и отслеживания прогресса выполнения сложных запросов.
    *   **📁 Работа с файлами:** Загрузка документов от пользователя (лимит 1GB на сессию), умные подсказки типов, скачивание файлов из песочницы пользователю.
    *   **Веб-поиск и извлечение данных:** Интеграция с Tavily API для получения актуальной информации из сети.
    *   **🔗 Система хуков (Hooks):** Расширяемая архитектура для перехвата и кастомизации поведения агента на различных этапах выполнения.
...
*   **🔒 Безопасность:** Использование `unsafe_code = forbid`, строгие Clippy lints, отсутствие небезопасного кода.
...
│   │   ├── providers/ # Провайдеры инструментов (Sandbox, Tavily, Todos)
│   │   ├── hooks/    # Система хуков (completion, registry, types)
...
│   └── main.rs       # Точка входа
├── tests/            # Интеграционные тесты (51 тестов)
├── sandbox/          # Ресурсы для образа песочницы (Python 3.12)
├── Cargo.toml        # Зависимости проекта (lazy-regex, aws-sdk-s3, async-openai)
├── Dockerfile        # Dockerfile основного приложения (Rust 1.92)
└── docker-compose.yml
```
...
- **Clippy lints:** Строгий режим (`unwrap_used = deny`)
