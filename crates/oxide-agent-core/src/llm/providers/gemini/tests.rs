...
mod gemini_tests {
    ...
    #[test]
    fn enables_high_thinking_for_gemini_3_1_flash_lite_preview() {
        let config = GeminiProvider::thinking_config_for_model("gemini-3.1-flash-lite-preview")
            .expect("expected thinking config");
    ...
    #[test]
    fn enables_high_thinking_for_normalized_gemini_preview_model_id() {
        let config =
            GeminiProvider::thinking_config_for_model("models/gemini-3.1-flash-lite-preview")
                .expect("expected thinking config");
    ...
    fn surfaces_blocked_prompt_when_no_text() {
        ...
        let err = GeminiProvider::extract_text_response(&response).expect_err("expected error");
        assert!(
    ...
    fn extracts_only_non_thought_text_from_mixed_parts() {
        ...
        let text = GeminiProvider::extract_text_response(&response).expect("expected text");
    ...
    fn parse_chat_response_surfaces_blocked_prompt_before_other_content() {
        ...
        let err = GeminiProvider::parse_chat_response(&response).expect_err("expected error");
        assert!(matches!(
            err,
