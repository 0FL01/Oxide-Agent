...
#[derive(Clone)]
struct PendingTextInputPart {
    message_id: MessageId,
    text: String,
}

struct PendingTextInputBatch {
...
struct BatchedTextInputCheck<'a> {
    msg: &'a Message,
    bot: &'a Bot,
...
#[derive(Clone)]
struct DeferredAgentInputContext {
    dispatch: BatchedTextTaskContext,
    llm: Arc<LlmClient>,
    msg: Message,
    sandbox_scope: SandboxScope,
}

struct RunningAgentMessageContext<'a> {
    msg: &'a Message,
    bot: &'a Bot,
    route: &'a TopicRouteDecision,
...
const AGENT_TEXT_INPUT_BATCH_DEBOUNCE_MS: u64 = 900;
const AGENT_TEXT_INPUT_SPLIT_THRESHOLD_CHARS: usize = 3500;
...
async fn clear_pending_text_batch(session_id: SessionId) {
...
async fn handle_batched_text_input_if_needed(ctx: BatchedTextInputCheck<'_>) -> Result<bool> {
...
fn build_batched_text_task_context(
    bot: &Bot,
...
async fn is_agent_mode_context(
    storage: &Arc<dyn StorageProvider>,
    user_id: i64,
    chat_id: ChatId,
    thread_spec: TelegramThreadSpec,
...
async fn ensure_agent_flow_session_keys(
    storage: &Arc<dyn StorageProvider>,
    user_id: i64,
    chat_id: ChatId,
    thread_spec: TelegramThreadSpec,
) -> Result<(String, bool, AgentModeSessionKeys)> {
    let (agent_flow_id, agent_flow_created) =
...
async fn handle_running_agent_message_if_needed(
    ctx: RunningAgentMessageContext<'_>,
) -> Result<bool> {
    if !is_agent_task_running(ctx.dispatch.session_id).await {
        return Ok(false);
    }
...
async fn configure_active_session(
    ctx: &ActiveSessionConfig,
    execution_profile: AgentExecutionProfile,
    topic_infra_config: Option<oxide_agent_core::storage::TopicInfraConfigRecord>,
) {
...
fn route_allows_agent_processing(route: &TopicRouteDecision, user_id: i64) -> bool {
        ...
        .await
        {
            Ok(task_text) => {
                    ...
                    dispatch_preprocessed_agent_text(ctx.dispatch.clone(), task_text).await
                {
                    ...
                    let _ = super::resilient::send_message_resilient_with_thread(
                        &ctx.dispatch.bot,
                        chat_id,
...
async fn preprocess_agent_message_input(
    bot: &Bot,
    msg: &Message,
    llm: &Arc<LlmClient>,
    sandbox_scope: &SandboxScope,
...
async fn dispatch_preprocessed_agent_text(
    ctx: BatchedTextTaskContext,
    task_text: String,
) -> Result<()> {
...
fn assemble_text_batch(parts: &[PendingTextInputPart]) -> String {
