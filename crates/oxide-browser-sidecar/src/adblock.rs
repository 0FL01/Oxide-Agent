...
mod tests {
    ...
    #[test]
    fn fetch_patterns_exclude_document() {
        let has_document = FETCH_PATTERNS.iter().any(|&(_, v)| v == "Document");
        assert!(
            !has_document,
            "Document must be excluded from Fetch patterns"
        );
    }
