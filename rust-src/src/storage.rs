...
impl R2Storage {
    ...
    pub async fn save_json<T: serde::Serialize>(&self, key: &str, data: &T) -> Result<(), StorageError> {
    ...
    pub async fn load_json<T: serde::de::DeserializeOwned>(&self, key: &str) -> Result<Option<T>, StorageError> {
        ...
        match result {
            Ok(output) => {
                let data = output.body.collect().await.map_err(|e| StorageError::Io(std::io::Error::other(e)))?;
    ...
    pub async fn get_user_config(&self, user_id: i64) -> Result<UserConfig, StorageError> {
        Ok(self.load_json(&user_config_key(user_id)).await?.unwrap_or_default())
    }

    pub async fn update_user_config(&self, user_id: i64, config: UserConfig) -> Result<(), StorageError> {
        self.save_json(&user_config_key(user_id), &config).await
    }

    pub async fn update_user_prompt(&self, user_id: i64, system_prompt: String) -> Result<(), StorageError> {
    ...
    pub async fn update_user_model(&self, user_id: i64, model_name: String) -> Result<(), StorageError> {
        let mut config = self.get_user_config(user_id).await?;
    ...
    pub async fn save_message(&self, user_id: i64, role: String, content: String) -> Result<(), StorageError> {
    ...
    pub async fn get_chat_history(&self, user_id: i64, limit: usize) -> Result<Vec<Message>, StorageError> {
        let history: Vec<Message> = self.load_json(&user_history_key(user_id)).await?.unwrap_or_default();
