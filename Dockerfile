...
COPY . .
RUN cargo build --release -p oxide-agent-telegram-bot -p oxide-agent-sandboxd --no-default-features -F oxide-agent-telegram-bot/profile-full -F oxide-agent-sandboxd/profile-full
