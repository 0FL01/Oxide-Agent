...
RUN cargo chef cook --release --workspace --no-default-features --features oxide-agent-telegram-bot/profile-full --recipe-path recipe.json

# Build application - this layer is rebuilt when source changes
COPY . .
RUN cargo build --release -p oxide-agent-telegram-bot -p oxide-agent-sandboxd --no-default-features -F oxide-agent-telegram-bot/profile-full
