...
**Статус:** предложен как альтернатива DR-002. Если принят, заменяет DR-002.

**TL;DR:** выкинуть chat APIs с корнями, без test-only shims и no-op методов. Всё, что реально нужно агенту, переписать на Agent Mode primitives: `chat_with_tools`/agent route, agent memory, agent flow, media route, topic/profile config.

**Решение**

Не оставлять compatibility shims вообще.

То есть не делать такого:

```rust
async fn get_chat_history(...) -> Ok(vec![])
async fn save_message_for_chat(...) -> Ok(())
async fn chat_completion(...) -> Err("removed")
```

Это плохой компромисс. Формально Chat Mode удалён, но контракт остаётся. Потом кто-то снова вызовет этот метод "потому что он есть", тесты пропустят, и chat runtime вернётся через боковую дверь.

Правильный подход:

```text
удалить chat методы из traits
↓
получить compile errors
↓
каждую ошибку классифицировать:
  - chat-only → удалить
  - реально agent feature → переписать на agent primitive
  - тестовый хелпер → переписать на agent mock
```

Это самый простой и надёжный способ.

**Что именно удалить с корнями**
...
Почему: это всё обслуживает Chat Mode: per-user prompt, per-user chat model, chat history, scoped chat UUID.
...
agent flow records
topic/profile/control-plane storage
reminders/audit/wiki/etc.
```

Если какой-то код после удаления просит `get_user_prompt()` — это сигнал, что он должен идти не в user chat prompt, а в agent profile/topic/agents.md config.

Если какой-то код просит `get_user_model()` — это сигнал, что модель должна браться из agent route selection, а не из per-user chat model.

**Что удалить из persisted structs**

В `UserConfig` и `UserContextConfig` удалить:
...
state
contexts
current_agent_flow_id
chat_id
thread_id
forum topic metadata
```

Старые JSON-конфиги с этими полями не нужно мигрировать. Serde по умолчанию игнорирует неизвестные поля, если нет `deny_unknown_fields`. Это как раз нормальный clean-on-read: старые chat-поля становятся мусором, runtime их не видит.

Важный нюанс: `generate_chat_uuid()` сейчас может переиспользоваться для agent flow ID. Его не обязательно удалять как генератор UUID, но надо **переименовать**:
...
```rust
generate_flow_id()
generate_runtime_uuid()
```

Иначе слово `chat` останется в agent runtime без причины.

**Что сделать с `transport-web/src/in_memory_storage.rs`**

Тут не нужен shim. Надо просто удалить поля:

```rust
user_prompts
user_models
chat_histories
chat_histories_by_chat
```

И удалить реализации всех chat methods.

Оставить in-memory storage только для того, что нужно web/session/e2e agent flow:

```rust
user_configs
agent_memories
agent_memories_context
agent_memories_flow
flow_records
wiki_objects
reminder_jobs
...
Комментарий в файле тоже поправить. Сейчас он говорит, что storage нужен для Agent Mode, но всё ещё перечисляет chat history. После cleanup должно быть примерно:

```rust
//! In-memory implementation of `StorageProvider` for Agent Mode E2E tests.
//!
//! Implements:
//! - user configs/state
//! - agent memory
...
**Что сделать с `LlmProvider`**

Тут тоже резать жёстко.

Сейчас trait заставляет каждого provider реализовать:

```rust
async fn chat_completion(...)
```

Это корень проблемы: даже agent-compatible provider обязан иметь plain chat path.

Удалить из `LlmProvider`:

```rust
chat_completion
```

Оставить как основной LLM path:

```rust
chat_with_tools
```

Плюс оставить media methods:
...
То есть базовый provider contract становится agent/media-first:
...
Можно оставить default error для media, но не для `chat_with_tools`. Для agent-only проекта provider без `chat_with_tools` не должен компилироваться как полноценный provider.

**Что сделать с `LlmClient`**

Удалить публичные методы:

```rust
LlmClient::chat_completion()
LlmClient::chat_completion_for_model_info()
```

И не заменять их на `internal_text_completion`, если хотим максимально clean agent-only.

Вместо этого внутренние агентные задачи переписать на `chat_with_tools` с пустым списком tools.

То есть было:

```rust
llm_client
    .chat_completion_for_model_info(system_prompt, &[], user_prompt, model)
    .await
```
...
Это не overengineering. Это просто использование одного agent LLM path для всех LLM-вызовов.

**Как переписать "оставшиеся" internal uses**

Текущие internal uses plain completion:
...
Перевести на agent route без tools.

Было: "дай plain summary через chat_completion".

Стало: "дай summary через agent-compatible model call, tools empty".

Важно: compaction не должна использовать chat model fallback. Она должна брать agent model / explicit compaction route, если такой есть.

**Loop detection**

Тоже через `chat_with_tools` с пустыми tools.

Если scout model не agent-compatible — fail fast или выключить LLM detector, но не fallback в chat provider.

**Wiki memory writer**

Тоже через agent route без tools.

Если нужен отдельный background model — он должен быть agent-compatible route, а не chat route.

**Input intent classifier**

Лучше вынести из Telegram transport в core/internal agent service, потому что Telegram не должен напрямую иметь LLM helper для классификации.

Но без большой архитектуры можно сделать минимально:

```text
bot/agent_handlers/input_intent.rs
  вызывает agent-safe LlmClient method
  не вызывает chat_completion
  не имеет доступа к chat history
```

Если есть силы — перенести позже.

**Нужен ли новый helper?**

Чтобы не размазывать extraction `ChatResponse.content` по четырём местам, можно добавить маленький helper, но не trait и не новый subsystem.

Например:

```rust
pub async fn agent_aux_text_response(
    &self,
    system_prompt: &str,
    messages: &[Message],
    user_message: &str,
...
```rust
agent_aux_text_response(...)
```

Смысл: это не user-facing chat. Это auxiliary agent call через тот же provider capability gate.

**Что делать с provider internals**

У провайдеров можно удалить trait method `chat_completion`, но внутри provider-кода могут остаться HTTP endpoint названия типа:
...
С этим не надо фанатично бороться, потому что многие upstream APIs реально называются `chat/completions`.

Прагматичная граница:

**Hard delete:**

```text
StorageProvider chat methods
LlmProvider::chat_completion
LlmClient::chat_completion
LlmClient::chat_completion_for_model_info
Telegram process_llm_request
chat history storage
per-user chat model/prompt
test mock chat APIs
```

**Допустимые остатки только в provider-private code:**

```text
openai_compat::chat_completions endpoint helper
provider request body naming, если upstream API так называется
```

Но они не должны быть доступны из transport/user runtime.

**Как обновить mocks**

`oxide-agent-core/src/testing.rs`

Удалить:

```rust
configure_chat_expectations()
expect_chat_completion()
```

Заменить `mock_llm_simple()` на agent mock.

Например:

```rust
mock_llm_agent_text("Done")
```
...
Если агентный runner ожидает structured JSON, тогда helper должен возвращать JSON-ответ:

```json
{"thought":"...","final_answer":"Done"}
```

Но это должно быть явно в названии helper-а:

```rust
mock_llm_agent_final_answer("Done")
```

А не универсальный `mock_llm_simple`.

`transport-web/src/scripted_llm.rs`
...
`ScriptedResponse::Text` уже хорошо ложится на Agent Mode: он может отдавать final answer в `ChatResponse`.

Telegram/web/core tests

Удалить тесты, которые проверяли:
...
Переписать только те тесты, которые проверяли реальную агентную функциональность, но случайно использовали chat mocks.

**Практичный порядок работ**

**Step 1: Remove storage chat contract**

Удалить chat methods из `StorageProvider`.

После этого компилятор покажет все места, где Chat Mode ещё жив.

Фиксить так:

```text
handlers/process_llm_request → delete
model selection/prompt editing → delete
context chat uuid → replace with agent flow id where needed
mocks → remove methods, not no-op
```

**Step 2: Remove storage chat data model**

Удалить:

```text
Message из storage/user.rs, если больше не используется
current_chat_uuid
system_prompt
model_name
chat history keys
R2 chat history methods
telemetry chat history category
```

Если `Message` нужен LLM — использовать `llm::Message`, не storage `Message`.

**Step 3: Remove LLM plain chat contract**
...
Provider implementations должны компилироваться только через:
...
**Step 4: Rewrite internal auxiliary calls**
...
```text
chat_with_tools(..., tools = [], json_mode = false/по задаче)
```

Если конкретная задача требует JSON, использовать `json_mode = true`, но только если route supports structured output.

**Step 5: Fix web/test mocks**

`InMemoryStorage` становится agent-only.

`ScriptedLlmProvider` становится agent-only.

`mock_storage_noop()` больше не конфигурирует chat expectations.

`mock_llm_simple()` либо удаляется, либо переименовывается в agent-specific helper.
...
```bash
rg -n "expect_chat_completion|mock_llm_simple|configure_chat_expectations" crates
rg -n "save_message_for_chat|get_chat_history|current_chat_uuid|update_user_model|get_user_model" crates
rg -n "LlmClient::chat_completion|chat_completion_for_model_info|process_llm_request" crates
```

Эти grep должны быть пустыми, кроме allowlist для private upstream endpoint helpers.

**Итоговый pragmatic выбор**

Удаляем Chat APIs из корневых traits. Не делаем shims. Всё, что осталось живым, переписываем на Agent Mode: storage → agent memory/flow/profile/topic; LLM → `chat_with_tools`; tests → agent mocks.
...
### Risk: old persisted `chat_mode` resurrects behavior
