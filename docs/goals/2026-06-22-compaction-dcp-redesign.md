...
- G2: One `CompactionEngine` is the only runtime mutation authority.
  ...
  - Status: in_progress
  - Evidence collected: Phase 6 wired compress tool through `CompactionEngine::apply_compression` in `runner/tools.rs::apply_compress_through_engine`. The compress tool executor is a pure parser (no memory access); the runner extracts `CompressRequest` from `structured_payload` and calls the engine. Old threshold-based `state.request_manual_compaction()` trigger removed from tool output path. `typed_runtime_compress_applies_through_engine` test proves block creation through engine. Other trigger paths (pre-sampling, context-limit, model-downshift, manual) still use old system — migration is Phase 7.
