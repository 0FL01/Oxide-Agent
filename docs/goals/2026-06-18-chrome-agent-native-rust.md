...
- G3: Single CDP WebSocket connection per session (no redundant CDPListener).
  ...
  - Status: verified
  - Evidence collected: CP2 — `CdpClient` (cdp.rs) owns exactly one `tokio-tungstenite` WebSocket per session. `BrowserSession` holds one `CdpClient` (session.rs:27). Background reader task dispatches both command responses (by `id` correlation via `HashMap<u64, oneshot::Sender>`) and events (via `broadcast::channel<CdpEvent>`) on the same connection. No separate CDPListener struct, no second WebSocket. `CdpClient::send_command` is `&self` (concurrent-safe via `Arc<Mutex<HashMap>>` + `mpsc::Sender`), allowing multiple concurrent commands on the single stream. `navigate_to` demonstrates the pattern: `Page.enable` + `Page.navigate` + `Page.loadEventFired` event all on one connection. Integration test confirms single-session lifecycle works end-to-end. CP5 — `CaptureCollector::start()` subscribes to the same `CdpClient`'s broadcast channel and sends `Network.enable` + `Log.enable` on the same WebSocket. Background event loop processes `Network.requestWillBeSent/responseReceived/loadingFinished/loadingFailed`, `Log.entryAdded`, `Page.frameNavigated` — all on the single connection. `drain_console_js()` calls `Runtime.evaluate` on the same connection to drain the JS interceptor. No second WebSocket, no separate listener task.
...
- G4: Stealth-safe capture — `Runtime.enable` never sent.
  ...
  - Status: verified
  - Evidence collected: CP3 — `stealth.rs` (78 lines) ports `STEALTH_PATCHES_JS` verbatim from chrome-agent `setup.rs`: navigator.webdriver=undefined, chrome.runtime mock, Permissions API fix, WebGL vendor/renderer mask (Intel Inc./Intel Iris OpenGL Engine), MouseEvent screenX/screenY leak fix. `apply_stealth()` calls `Page.addScriptToEvaluateOnNewDocument` (survives navigations) + `Runtime.evaluate` (patches current page) + `Network.setUserAgentOverride` (removes HeadlessChrome, replaces with Chrome/131.0.0.0). `grep -r 'Runtime.enable' crates/oxide-browser-sidecar/src/` → only in comments/assertions, never as a CDP command. `grep '"Runtime' crates/oxide-browser-sidecar/src/` → only `Runtime.evaluate` (stealth.rs:92). Integration test `snapshot_and_stealth_on_real_chromium` verifies: (1) `navigator.webdriver` is undefined/null, (2) UA does not contain "HeadlessChrome". CP5 — `capture.rs` (1248 lines) implements network/console capture entirely without `Runtime.enable`: (1) Network events via `Network.enable` on the session WebSocket; (2) Browser-level log entries via `Log.enable` (independent domain, no `Runtime.enable` needed); (3) JS `console.log/warn/error` calls via injected JS interceptor (`CONSOLE_INTERCEPTOR_JS`) installed via `Page.addScriptToEvaluateOnNewDocument` + `Runtime.evaluate`, drained on demand via `Runtime.evaluate` calling `window.__oxideDrainConsole()`. Integration test `capture_network_and_console_on_real_chromium` on Chrome/149 verifies: JS `console.warn`/`console.error` captured by interceptor (no `Runtime.enable`), `console.log`/info filtered (only Warning/Error in contract), failed fetch network request captured, favicon.ico noise filtered. `grep 'Runtime.enable' crates/oxide-browser-sidecar/src/` → only comments/assertions, never as CDP command ✓.
...
- G5: REST contract with Oxide client unchanged.
  ...
  - Status: verified
  - Evidence collected: CP6 — all 9 REST endpoints implemented in `lib.rs`: `GET /healthz` (no auth, `{"ok":true,"native":true}`), `POST /sessions` (create session), `DELETE /sessions/{id}` (close session), `POST /sessions/{id}/goto` (navigate with SPA hash detection, force_reload, wait_until=networkidle polling, capture_after observation), `GET /sessions/{id}/observe` (concurrent a11y+screenshot+URL/title via tokio::join!, optional DOM snapshot, network/console drain+merge+summarize, fresh/cached observation), `POST /sessions/{id}/action` (execute action, post-observation with 200ms drain delay), `GET /sessions/{id}/screenshot/latest` (metadata JSON or binary image/png bytes), `GET /sessions/{id}/debug/network` (filtered network history), `GET /sessions/{id}/debug/console` (filtered console history). All endpoints use shared types from `oxide-browser-contracts` — no contract drift possible. `client.rs` unchanged (N1 verified in CP1). Integration test `full_rest_contract_on_real_chromium` on Chrome/149 verifies: healthz, create session, goto with capture_after, observe with DOM, click_selector action with post_observation, fill action, screenshot/latest metadata + binary (PNG signature verified), debug/network, debug/console, close session, session-not-found after close. 3 integration tests pass in 1.61s. Serde defaults added to contracts crate (`ScreenshotFormat`, `DebugLevel`, `NetworkFilter`, `ConsoleLevel` implement `Default`; `max_debug_items` and `limit` default to 20) — matches Python sidecar's query param defaults, purely additive (deserialization only), zero impact on client serialization.
...
- 2026-06-18 (CP5): Console capture uses a JS interceptor instead of `Runtime.consoleAPICalled`. Rationale: `Runtime.consoleAPICalled` requires `Runtime.enable` which is a stealth detection vector. The JS interceptor overrides `console.log/warn/error/debug/info`, stores entries in `window.__oxideConsoleCapture`, and is drained on demand via `Runtime.evaluate` (which works without `Runtime.enable`, verified in CP0). The interceptor is injected via `Page.addScriptToEvaluateOnNewDocument` (survives navigations) + `Runtime.evaluate` (current page). This replaces the Python sidecar's `CDPListener._on_console_api_called` which required `Runtime.enable`.
- 2026-06-18 (CP5): Capture collector runs on the session's CDP WebSocket, not a separate connection. Rationale: the Python sidecar's `CDPListener` opens a SECOND WebSocket to the page target — this is the root architectural problem (G3 violation). The native `CaptureCollector::start()` subscribes to the existing `CdpClient`'s broadcast channel and sends `Network.enable`/`Log.enable` on the same connection. The background event loop processes events from the same broadcast receiver. Body capture (`Network.getResponseBody`) is also sent on the same connection via `cdp.send_command()`. Single connection, no redundancy.
- 2026-06-18 (CP5): `ConsoleLevel` contract enum only has `Warning` and `Error`. Info/verbose/debug console messages are filtered at capture time, not stored. Rationale: the contract type `ConsoleItem.level` is `ConsoleLevel` which only has `Warning`|`Error`. The Python sidecar stored all levels as strings and filtered when building payloads, but the Rust contract's type system enforces Warning|Error only. Filtering at capture time prevents accumulating items that can't be serialized.
...
- 2026-06-18 19:00: CP5 complete — Capture (network/console, stealth-safe).
  - Changed: `crates/oxide-browser-sidecar/src/capture.rs` (new — 1248 lines: `CaptureCollector` struct with network_items/console_items/pending_requests/current_url state, `start()` subscribes to CDP events + sends Network.enable + Log.enable + injects console interceptor via Page.addScriptToEvaluateOnNewDocument + Runtime.evaluate, background event loop processing Network.requestWillBeSent/responseReceived/loadingFinished/loadingFailed + Log.entryAdded + Page.frameNavigated, body capture via Network.getResponseBody for XHR/fetch/failed, `drain_network()`/`drain_console()`/`drain_console_js()` free function, `is_network_failure`/`is_noise_network`/`is_noise_console`/`resource_type_from_cdp`/`should_capture_body`/`decode_response_body` ported from Python, `summarize_network`/`summarize_console`/`build_network_debug_payload`/`build_console_debug_payload`/`merge_network_history`/`merge_console_history` ported from Python, `now_iso()`/`epoch_to_iso()` without external deps, 27 unit tests); `crates/oxide-browser-sidecar/tests/capture_integration.rs` (new — 2 integration tests on real Chromium: `capture_network_and_console_on_real_chromium` verifies JS console.warn/error captured + console.log filtered + failed fetch captured + favicon noise filtered; `capture_summarize_and_debug_payloads` verifies summarize + debug payload building); `crates/oxide-browser-sidecar/Cargo.toml` (added `base64 = "0.22"` for response body decoding); `crates/oxide-browser-sidecar/src/lib.rs` (added `pub mod capture;`); `crates/oxide-browser-sidecar/src/session.rs` (integrated `CaptureCollector` into `BrowserSession` — starts before navigation, stored as `pub capture: Arc<CaptureCollector>`); `docs/goals/2026-06-18-chrome-agent-native-rust.md` (this file — G3 evidence +CP5, G4 evidence +CP5, Decisions +3, Progress Log +CP5).
  - Evidence: `cargo clippy -p oxide-browser-sidecar --all-targets -- -D warnings` ✓; `cargo fmt --all -- --check` ✓; `cargo test -p oxide-browser-sidecar` — 76 unit tests passed, 5 integration tests ignored ✓; `cargo test -p oxide-browser-sidecar --test capture_integration -- --ignored --nocapture` — 2 tests passed in 1.27s ✓ (console.warn/error captured via JS interceptor, console.log filtered, failed fetch captured, favicon noise filtered, summarize + debug payloads built); `cargo test -p oxide-browser-sidecar --test cdp_integration -- --ignored --nocapture` — passed ✓; `cargo test -p oxide-browser-sidecar --test snapshot_stealth -- --ignored --nocapture` — passed ✓; `cargo test -p oxide-browser-sidecar --test actions_integration -- --ignored --nocapture` — passed ✓; `cargo check -p oxide-agent-core --no-default-features --features profile-full` ✓; `cargo clippy -p oxide-agent-core --no-default-features --features profile-full --all-targets -- -D warnings` ✓; `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- browser_live` — 72 passed, 1 ignored ✓; `grep 'Runtime.enable' crates/oxide-browser-sidecar/src/` → only comments/assertions, never as CDP command ✓. P0.5 bugs caught during CP5: (1) `if let` chains needed for Rust 2024 edition to satisfy clippy `collapsible_if` — collapsed nested `if should_capture_body { if let Ok(result) = ... { if let Some(body) = ... } }` into single `if should_capture_body && let Ok(result) = ... && let Some(body) = ...`; (2) `filter(is_network_failure)` fails because `iter().filter()` receives `&&NetworkItem` (double reference) while `is_network_failure` takes `&NetworkItem` — used closure `|i| is_network_failure(i)` for `filter`, direct function for `retain` (which passes `&NetworkItem`); (3) `#![allow(clippy::unwrap_used)]` in test file conflicts with crate-level `forbid(unwrap_used)` — replaced all `.unwrap()` with `.expect()` in tests.
  - Commands: see above.
  - Audit IDs updated: G3 capture-side evidence added (verified), G4 capture-side evidence added (verified).
  - Next: CP6 — REST server (full contract) + observation building + DOM snapshot + artifacts.

- 2026-06-18 20:00: CP6 complete — REST server (full contract) + observation building + DOM snapshot + artifacts.
  - Changed: `crates/oxide-browser-contracts/src/types.rs` (added `#[derive(Default)]` + `#[default]` to `ScreenshotFormat`, `DebugLevel`, `NetworkFilter`, `ConsoleLevel`; added `#[serde(default)]` to `ScreenshotQuery::format`, `ObserveQuery::max_debug_items`, `NetworkDebugQuery::{level,filter,limit}`, `ConsoleDebugQuery::{level,min_level,limit}`; added `default_limit()` const fn returning 20 — matches Python sidecar's query param defaults, purely additive deserialization defaults); `crates/oxide-browser-sidecar/Cargo.toml` (added `sha2 = "0.10.9"`, added `query` feature to reqwest); `crates/oxide-browser-sidecar/src/dom.rs` (new — DOM snapshot JS ported verbatim from Python `DOM_SNAPSHOT_SCRIPT`, `capture_dom_snapshot()` via Runtime.evaluate with JSON parsing + error handling, 4 unit tests); `crates/oxide-browser-sidecar/src/screenshot.rs` (new — `capture_screenshot()` via Page.captureScreenshot with base64 decode + disk save + sha256 + artifact URI, `read_latest_screenshot()` for binary endpoint, `session_artifact_dir()` + `resolve_artifact_dir()`, ONE_PIXEL_PNG fallback, 5 unit tests); `crates/oxide-browser-sidecar/src/observe.rs` (new — `build_observation()` with concurrent a11y+screenshot+URL/title via `tokio::join!` (~1.6x speedup from CP0), optional DOM snapshot, network/console drain+merge+summarize, fresh/cached observation, `get_url_title()` via single Runtime.evaluate, `title_from_a11y()` fallback, 3 unit tests); `crates/oxide-browser-sidecar/src/session.rs` (restructured: `BrowserInner` struct behind `tokio::sync::Mutex` for force_reload support, `BrowserSession` with state fields — url, title, last_screenshot, last_observation, network_history, console_history, observation/screenshot seq counters; `cdp()`/`capture()`/`page_id()` async accessors clone out of inner; `force_reload()` method replaces Chromium+CDP+capture+page_id atomically; `safe()` and `new_request_id()` made pub); `crates/oxide-browser-sidecar/src/capture.rs` (added `pub fn pending_request_count()` for networkidle polling, `now_iso()` made pub); `crates/oxide-browser-sidecar/src/lib.rs` (all 9 REST handlers implemented: healthz, create/close session, goto with SPA hash navigation + force_reload + networkidle wait, observe with query params, action with post-observation, screenshot/latest metadata+binary, debug/network, debug/console; `is_same_origin_path_hash_navigation()`, `wait_for_network_idle()`, `action_kind_str()`, `not_found_error()`, `empty_observation()`, `empty_screenshot()` helpers; 6 unit tests); `crates/oxide-browser-sidecar/tests/rest_contract.rs` (new — 3 integration tests: `full_rest_contract_on_real_chromium` verifies all 9 endpoints end-to-end, `goto_force_reload_works`, `action_get_element_value_returns_result`); `docs/goals/2026-06-18-chrome-agent-native-rust.md` (this file — G5→verified, G6→verified, Decisions +CP6, Progress Log +CP6).
  - Evidence: `cargo clippy -p oxide-browser-contracts -p oxide-browser-sidecar --all-targets -- -D warnings` ✓; `cargo fmt --all -- --check` ✓; `cargo test -p oxide-browser-contracts --lib` — 4 passed ✓; `cargo test -p oxide-browser-sidecar` — 93 unit tests passed, 8 integration tests ignored ✓; `cargo test -p oxide-browser-sidecar --test rest_contract -- --ignored --nocapture` — 3 tests passed in 1.61s ✓ (full REST contract: healthz, create session, goto with capture_after, observe with DOM, click_selector with post_observation, fill, screenshot metadata + binary with PNG signature, debug/network, debug/console, close, not_found after close; force_reload; get_element_value with result string); `cargo test -p oxide-browser-sidecar --test cdp_integration -- --ignored --nocapture` — passed ✓; `cargo test -p oxide-browser-sidecar --test snapshot_stealth -- --ignored --nocapture` — passed ✓; `cargo test -p oxide-browser-sidecar --test actions_integration -- --ignored --nocapture` — passed ✓; `cargo test -p oxide-browser-sidecar --test capture_integration -- --ignored --nocapture` — passed ✓; `cargo check -p oxide-agent-core --no-default-features --features profile-full` ✓; `cargo clippy -p oxide-agent-core --no-default-features --features profile-full --all-targets -- -D warnings` ✓; `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- browser_live` — 72 passed, 1 ignored ✓. P0.5 bugs caught during CP6: (1) `reqwest::RequestBuilder::query` is behind the `query` feature flag in reqwest 0.13.4 — added `query` to features list; (2) `std::env::set_var` is unsafe in Rust 2024 and crate forbids `unsafe_code` — refactored `session_artifact_dir` to split env var reading from path construction (`resolve_artifact_dir` takes root as parameter, testable without env manipulation); (3) `take_snapshot` returns `Result<SnapshotResult, SnapshotError>` not `SnapshotResult` directly — added error handling in `build_observation` (empty a11y_summary on error); (4) `serde_json::from_str` takes `&str` not `String` — added `&` borrow.
  - Commands: see above.
