...
- 2026-05-26 22:28 +03: Completed provider route-policy checkpoint. OpenRouter provider-level tools/media now default-deny; route compatibility comes from a static model policy for the PRD-sanctioned OpenRouter model IDs, with Gemini-family routes approved only for media and DeepSeek V4 OpenRouter routes approved for main Agent Mode. OpenRouter tool requests now set `require_parameters=true`. NVIDIA NIM now uses the PRD exact-match allowlist (`deepseek-ai/deepseek-v4-flash`, `deepseek-ai/deepseek-v4-pro`) with no wildcard family matching, provider-level NVIDIA capabilities default-deny, route selection checks model capabilities before choosing a route, and ChatGPT JSON-mode restrictions cover `chatgpt`, `openai-chatgpt`, and `llm-provider/openai-chatgpt`. Config validation now rejects unapproved agent routes and media routes before credential validation. Updated README and `.env.example` NVIDIA examples to the allowlisted model. Verified `cargo fmt`, `cargo test -p oxide-agent-core --no-default-features --features profile-full config::tests --lib`, `cargo test -p oxide-agent-core --no-default-features --features profile-full llm::capabilities::tests --lib`, `cargo test -p oxide-agent-core --no-default-features --features profile-full llm::client::tests --lib`, `cargo test -p oxide-agent-core --no-default-features --features profile-full llm::providers::modules::tests --lib`, `cargo test -p oxide-agent-core --no-default-features --features profile-full llm::providers::openrouter::tests --lib`, `cargo test -p oxide-agent-core --no-default-features --features profile-full llm::providers::nvidia::tests --lib`, `cargo test -p oxide-agent-core --no-default-features --features profile-full agent::runner::execution::tests::json_mode_forbids_chatgpt_routes_only --lib`, `cargo test -p oxide-agent-core --no-default-features --features profile-lite agent::runner::execution::tests::run_unstructured_mode_parses_accidental_structured_final_answer --lib`, `cargo test -p oxide-agent-core --no-default-features --features profile-lite agent::runner::execution::tests::run_skips_unsupported_nvidia_route_and_uses_backup --lib`, `cargo check --workspace --no-default-features --features profile-lite`, `cargo check -p oxide-agent-core --no-default-features --features profile-full`, `cargo clippy --workspace --no-default-features --features profile-lite`, `scripts/check-runtime-env-surface.sh`, and PRD grep guards. Next checkpoint: final validation sweep and decision on whether broad profile-lite unit-test failures are remaining pre-existing feature-matrix debt or must be fixed in this goal.
- 2026-05-26 22:33 +03: Final validation sweep is not yet green. `cargo test -p oxide-agent-core --no-default-features --features profile-lite --lib` currently reports 627 passed / 33 failed. The failures cluster around tests that still use mock or ad-hoc providers after default-deny capabilities (`mock`, `primary`, `backup`, ChatGPT route selection without `llm-chatgpt` in profile-lite), a media test that configures OpenRouter media while `profile-lite` does not enable `llm-openrouter`, and existing profile-lite feature-surface expectations for sandbox/SSH/control-plane tools. This is the next implementation slice; goal remains active.
