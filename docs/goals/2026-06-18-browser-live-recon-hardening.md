...
- Status: verified
- Evidence collected: CP-4 made `browser_extract` DOM extraction selector-safe and attribute-deterministic. The generated expression now passes the JSON-encoded CSS selector directly to `document.querySelectorAll(...)` instead of trying to `JSON.parse` a CSS selector string, collects normalized DOM `properties` (`value`, `innerText`, `innerHTML`, `textContent`, resolved `href`) plus all element attributes, and maps the requested `attribute` to `matches[].value` with `attribute_source`, `found`, `tag`, `index`, raw `properties`, and raw `attributes` diagnostics. Unit tests cover property extraction for `attribute:"value"`, DOM attribute extraction for `data-*`, selector expression generation without `JSON.parse`, and property-vs-attribute precedence. The live ignored OTS test `live_ots_browser_extract_returns_share_url_value_attribute` created a secret via Browser Live, waited for the share URL input, then used `browser_extract` with `selector:"input.form-control:not(#createSecretFiles)"` and `attribute:"value"`; it passed and asserted the extracted `matches[].value` starts with `https://ots.bash.md/#`, `attribute:"value"`, `attribute_source:"property"`, and `found:true`. An earlier run of the same live test failed with `Failure("action failed")`; inspecting the live session showed the share URL existed, and the root cause was the invalid generated `JSON.parse("input.form-control:not(...) ")` selector expression, which CP-4 removed.
...
- Status: verified
- Evidence collected: CP-5 made action observation categories explicit: `GetElementValue` and `Wait` are result-only (`capture_after:false`), `ExecuteJavaScript` and all visual actions capture fresh post-observations, and `Script` captures if any step needs it. Sidecar `capture_dom_snapshot` now returns `(snapshot, error)` with structured `SidecarErrorBody`-shaped errors on all failure paths (pipe unavailable, eval failure, empty result, invalid JSON, invalid shape); `build_observation` propagates `dom_snapshot_error` in observations. Tool output includes `post_observation_diagnostics` with `mode`/`source`/`fresh_observation`/`fresh_screenshot`/`action_seq_current`/`dom_snapshot.status` (captured/captured_empty/error). Unit tests cover `execute_javascript` freshness diagnostics, `get_element_value` result-only behavior, stale post-observation diagnosis, and DOM snapshot error reporting. Live OTS probe on refreshed `oxide_chrome_agent_sidecar` confirmed: `get_element_value` with `capture_after:false` returned `ok:true`, share URL extracted (len 80), `post_observation:None` (result-only); `execute_javascript` DOM mutation returned `ok:true`, result `'mutation-done'`, fresh `post_observation` with `observation_id:obs-br-d51b0a7bfda0-5`, `action_seq:5`, `dom_snapshot_len:12`, `dom_snapshot_error:None`. Sidecar self-test passed including new DOM snapshot capture and failure contract assertions.
...
- Status: in_progress
- Evidence collected: CP-0 docs-only checkpoint ran `python3 -m py_compile docker/chrome-agent-sidecar.py` (pass) and `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- agent::providers::browser_live --quiet` (65 passed, 0 failed). CP-1 ran `python3 -m py_compile docker/chrome-agent-sidecar.py` (pass), a temp-dir Python import/contract check for `action_to_pipe_cmd` (pass after documenting the local `/var/lib/oxide-browser` permission issue), `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- agent::providers::browser_live --quiet` (65 passed, 0 failed), `docker exec oxide_chrome_agent_sidecar chrome-agent-sidecar --self-test` (pass), and the live OTS semantic input probe (pass). CP-2 ran `python3 -m py_compile docker/chrome-agent-sidecar.py` (pass), a temp-dir Python import/source contract check asserting no reload JS remains and hash-navigation classification works (pass), `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- agent::providers::browser_live --quiet` (65 passed, 0 failed), refreshed the live sidecar with an executable script, `docker exec oxide_chrome_agent_sidecar chrome-agent-sidecar --self-test` (pass), a raw chrome-agent close/reopen contract probe (pass), and a live OTS force-reload reveal probe (pass). CP-3 ran `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- agent::providers::browser_live --quiet` (67 passed, 0 failed), `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib --quiet` (1305 passed, 8 ignored, 0 failed), and `cargo fmt --all -- --check` (pass after applying `cargo fmt --all`). CP-4 ran focused DOM extraction tests `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- agent::providers::browser_live::tools::tests::browser_extract agent::providers::browser_live::tools::tests::dom_extract agent::providers::browser_live::tools::tests::select_dom --quiet` (6 passed), live ignored OTS Browser Live extraction test with `OXIDE_BROWSER_LIVE_E2E=1` (1 passed after the selector-expression fix), `python3 -m py_compile docker/chrome-agent-sidecar.py` (pass), `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- agent::providers::browser_live --quiet` (70 passed, 1 ignored, 0 failed), `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib --quiet` (1308 passed, 9 ignored, 0 failed), `cargo fmt --all -- --check` (pass), and `docker exec oxide_chrome_agent_sidecar chrome-agent-sidecar --self-test` (pass). CP-5 ran `python3 -m py_compile docker/chrome-agent-sidecar.py` (pass), `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- agent::providers::browser_live --quiet` (76 tests, 75 passed, 1 ignored, 0 failed), `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib --quiet` (1313 passed, 9 ignored, 0 failed), `cargo fmt --all -- --check` (pass), `cargo clippy --workspace --all-targets -- -D warnings` (0 warnings), `docker exec oxide_chrome_agent_sidecar chrome-agent-sidecar --self-test` (pass including DOM snapshot capture/failure contract assertions), and a live OTS probe confirming `get_element_value` result-only and `execute_javascript` fresh post-observation (pass).
...
- 2026-06-18: Make `browser_extract` DOM output attribute-deterministic: the requested property/attribute becomes `matches[].value`, while raw `properties` and `attributes` remain present as diagnostics. This removes the need for ad-hoc page JavaScript to read normal form values such as the OTS share URL.
- 2026-06-18: Classify browser actions into result-only and capture-after categories. `GetElementValue` and `Wait` are result-only because they read scalars or wait for conditions without producing visual evidence. `ExecuteJavaScript` always captures because the receiver cannot prove arbitrary expressions are read-only. The sidecar must return either a fresh DOM snapshot or a structured `dom_snapshot_error` — silent `None` on failure paths is a contract bug, not graceful degradation.
...
- 2026-06-18 03:15: CP-5 observation freshness and diagnostics completed.
  - Changed: `actions.rs` now uses `needs_post_action_observation` to classify actions: `GetElementValue`/`Wait` are result-only, `ExecuteJavaScript` and all visual actions capture fresh post-observations, `Script` captures if any step needs it. `types.rs`/`session.rs` added `dom_snapshot_error` to `BrowserObservation`/`BrowserFrame`. `tools.rs` adds `post_observation_diagnostics` to tool output with `mode`/`source`/`fresh_observation`/`fresh_screenshot`/`action_seq_current`/`dom_snapshot.status`. `docker/chrome-agent-sidecar.py` `capture_dom_snapshot` returns `(snapshot, error)` with structured errors on all failure paths; `build_observation` propagates `dom_snapshot_error`; all 4 call sites updated; self-test extended with DOM snapshot capture/failure assertions.
  - Evidence: live OTS probe on refreshed `oxide_chrome_agent_sidecar` confirmed `get_element_value` with `capture_after:false` returned `ok:true`, share URL extracted (len 80), `post_observation:None`; `execute_javascript` DOM mutation returned `ok:true`, result `'mutation-done'`, fresh `post_observation` with `observation_id:obs-br-d51b0a7bfda0-5`, `action_seq:5`, `dom_snapshot_len:12`, `dom_snapshot_error:None`. Sidecar self-test passed.
  - Commands: `python3 -m py_compile docker/chrome-agent-sidecar.py`; `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib -- agent::providers::browser_live --quiet`; `cargo test -p oxide-agent-core --no-default-features --features profile-full --lib --quiet`; `cargo fmt --all -- --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `install -m 0755 docker/chrome-agent-sidecar.py target/chrome-agent-sidecar-live`; `docker cp target/chrome-agent-sidecar-live oxide_chrome_agent_sidecar:/usr/local/bin/chrome-agent-sidecar`; `docker restart oxide_chrome_agent_sidecar`; `docker exec oxide_chrome_agent_sidecar chrome-agent-sidecar --self-test`; live OTS probe via `docker exec -i oxide_chrome_agent_sidecar python3 -`.
  - Audit IDs updated: G6 verified; Q1/Q2 in progress.
  - Next: commit CP-5, record commit hash, then start CP-6 docs and final OTS E2E.
