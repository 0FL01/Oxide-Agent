...
- Q3: Observability and logging
  ...
  - Status: verified
  - Evidence collected: `browser_live::metrics::BrowserMetricsCollector` provides atomic counters for sessions, actions, observations, screenshots, MiMo latency/requests/errors/repair/invalid JSON, recovery attempts/safe stops, sidecar latency/requests/errors, and artifact byte sizes; `browser_live::tools` records these counters per tool call and returns the final snapshot in `browser_close`; `browser_live::mimo` records MiMo latency and `TokenUsage` including `cached_tokens` and `cache_creation_tokens`; `tracing` spans/field logs carry `session_id`, `task_id`, and `action_seq` without screenshot bytes or secrets. Tests: `browser_close_returns_metrics_snapshot`, `browser_step_populates_provider_metrics`, `browser_progress_and_outputs_exclude_screenshot_bytes`, `collector_tracks_*`, `mimo_decider_records_metrics_and_token_usage`, `mimo_decider_records_repair_and_invalid_json_metrics`, `mimo_decider_records_error_metric`.
...
- 2026-06-16: CP-15 Observability, metrics, and logging
  - Changed: added `browser_live::metrics::BrowserMetricsCollector` with atomic counters for sessions, actions, observations, screenshots, MiMo latency/requests/errors/repair/invalid JSON, recovery attempts/safe stops, sidecar latency/requests/errors, and artifact byte sizes; wired metrics into `BrowserLiveProvider` per tool call with final snapshot returned by `browser_close`; added `LlmProvider::analyze_image_with_usage` and `LlmClient::analyze_image_with_usage` to capture `TokenUsage` from browser MiMo calls; implemented usage extraction in `OpenCodeGoProvider`; updated `BrowserMimoDecider` to record MiMo latency and cached tokens; added `tracing` spans/field logs on browser tool methods carrying `session_id`, `task_id`, and `action_seq`; added `byte_size` to `ScreenshotArtifact` for accurate artifact-size metrics.
  - Evidence: metrics collector tests prove all counters and snapshots; `browser_close_returns_metrics_snapshot` and `browser_step_populates_provider_metrics` prove provider-level metrics are populated for every step; `mimo_decider_records_metrics_and_token_usage`/`mimo_decider_records_repair_and_invalid_json_metrics`/`mimo_decider_records_error_metric` prove MiMo latency, token, cached-token, repair, and error accounting; `browser_progress_and_outputs_exclude_screenshot_bytes` proves tool outputs and progress events contain no base64 or `data:image` bytes; `snapshot_serialization_excludes_sensitive_bytes_and_urls` proves metrics snapshots contain no URLs or image bytes.
