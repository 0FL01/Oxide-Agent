...
- 2026-05-25 21:28 +03: Pinned bwrap per-scope system overlay directories in scope metadata. Existing scopes now keep using the same external `BWRAP_ROOT_UPPER_DIR` per-scope directory after the environment changes, and admin deletion removes the metadata-pinned system directory rather than a newly computed path. Extended upperdir lifecycle tests for env-change pinning and delete cleanup. Verified focused override tests, full `bwrap::tests`, `cargo check -p oxide-agent-core --no-default-features --features sandbox-backend-bwrap`, `cargo clippy --workspace --no-default-features --features profile-host-bwrap`, and `cargo fmt --check`.
- 2026-05-25 21:34 +03: Hardened explicit `BWRAP_RESOLV_CONF=<path>` handling so resolver overrides must be regular files and direct symlinks are rejected before bwrap bind setup; `auto` mode remains unchanged. Documented the resolver path constraint. Verified focused config errors, full `bwrap::tests`, `cargo check -p oxide-agent-core --no-default-features --features sandbox-backend-bwrap`, `cargo clippy --workspace --no-default-features --features profile-host-bwrap`, and `cargo fmt --check`.
- 2026-05-25 21:39 +03: Hardened `BWRAP_STATE_DIR` and `BWRAP_LOCK_DIR` validation so existing paths must be real directories and direct symlink paths are rejected before scope state or lock files are created. Documented the state/lock directory constraint. Verified focused state/lock config errors, full `bwrap::tests`, `cargo check -p oxide-agent-core --no-default-features --features sandbox-backend-bwrap`, `cargo clippy --workspace --no-default-features --features profile-host-bwrap`, and `cargo fmt --check`.
- 2026-05-25 21:46 +03: Changed `BWRAP_RESOLV_CONF=auto` to copy the host resolver config into the bwrap scope state and bind-mount the staged regular file, avoiding direct host `/etc/resolv.conf` bind behavior when the host path is a symlink. Invocation tests now assert the auto bind source is scope-local. Verified focused invocation coverage, full `bwrap::tests`, `cargo check -p oxide-agent-core --no-default-features --features sandbox-backend-bwrap`, `cargo clippy --workspace --no-default-features --features profile-host-bwrap`, and `cargo fmt --check`.
...
- `cargo test -p oxide-agent-core --no-default-features --features sandbox-backend-bwrap bwrap_invocation_args_encode_network_root_modes_and_bind_policy --lib` passed with resolver auto-staging coverage.
...
- `cargo test -p oxide-agent-core --no-default-features --features sandbox-backend-bwrap bwrap_state_and_lock_dir_errors_name_config_keys --lib` passed with state/lock file and symlink rejection coverage.
...
- `cargo test -p oxide-agent-core --no-default-features --features sandbox-backend-bwrap bwrap_config_errors_are_actionable --lib` passed with explicit resolver directory and symlink rejection coverage.
