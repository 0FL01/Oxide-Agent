...
- 2026-05-22 21:52: Phase 8b added an active-runner fail-fast guard test for typed runtime contexts whose active route is not the v1-supported provider/model pair. The test verifies that the runner returns an unsupported-route error before tool execution and leaves assistant/tool history empty, preventing partial history writes or silent legacy fallback. Validation passed: `cargo test -p oxide-agent-core typed_runtime_path_rejects_unsupported_active_route_without_history_write -- --nocapture`, `cargo check -p oxide-agent-core`, `cargo fmt --all --check`, `cargo clippy -p oxide-agent-core --all-targets --all-features`. Next: decide and implement the final unsupported-route policy for non-v1 configurations, then remove or hard-disable legacy active execution if the PRD remains strict v1-only.
- 2026-05-22 22:07: Phase 8c hard-disabled active runner fallback to legacy tool execution. Native and structured tool-call branches now require a typed runtime registry and otherwise fail before writing assistant/tool history. Removed old runner-side `execute_tools`, hook-gated dispatch, sequential compress handling, approval-string detection, blocked-result writes, and related dead code from `runner/tools.rs` / `runner/hooks.rs`. Static check now finds no active runner references to `execute_tools(`, `record_tool_execution_result`, `APPROVAL_PENDING`, `Waiting for approval`, `ToolHookDecision`, or `apply_before_tool_hooks`. Validation passed: `cargo test -p oxide-agent-core tool_calls_without_typed_runtime_do_not_fallback_to_legacy_execution -- --nocapture`, `cargo test -p oxide-agent-core tool_runtime -- --nocapture`, `cargo check -p oxide-agent-core`, `cargo fmt --all --check`, `cargo clippy -p oxide-agent-core --all-targets --all-features`. Next: migrate or remove executor-level `tool_bridge` replay/resume paths and tighten v1 route selection before LLM calls.
- 2026-05-22 22:16: Phase 8d removed executor-level legacy bridge replay for SSH approvals. `resume_ssh_approval` is now explicitly disabled for typed runtime v1, `ExecutionRequest::ResumeApproval` and replay payload fields were removed, `replay_initial_tool_call` and imports of `tool_bridge::execute_single_tool_call` / legacy `ToolExecutionContext` were deleted, and the resume tests now assert the disabled v1 behavior. `tool_model_route` is now compiled only for the browser-use feature/test scope where it is still referenced. Validation passed: `cargo test -p oxide-agent-core resume_ssh_approval_is_disabled_for_typed_runtime_v1 -- --nocapture`, `cargo check -p oxide-agent-core`, `cargo fmt --all --check`, `cargo clippy -p oxide-agent-core --all-targets --all-features`. Next: remove or replace remaining `tool_bridge` dependencies used only for after-hook DTOs and todo sync helpers, then add static no-active-bridge guards.
- 2026-05-22 22:17: Phase 8e removed the legacy `agent/tool_bridge.rs` module entirely. After-tool hooks now receive direct `tool_name`/`output` values from typed runtime outputs, todo synchronization is local to runner response handling, `PendingSshReplay` and session/context replay storage were deleted, and SSH approval rejection no longer clears replay payloads. Added `tool_runtime_static_guards.rs` to keep the legacy bridge module unexported/removed. Static source check finds no `tool_bridge`, `ToolExecutionResult`, `PendingSshReplay`, `execute_single_tool_call`, or pending replay APIs in `crates/oxide-agent-core/src`. Validation passed: `cargo test -p oxide-agent-core legacy_tool_bridge_module_is_removed -- --nocapture`, `cargo check -p oxide-agent-core`, `cargo fmt --all --check`, `cargo clippy -p oxide-agent-core --all-targets --all-features`. Next: tighten final strict v1 route behavior before LLM calls and run broader integration verification.
- 2026-05-22 22:18: Phase 8f tightened v1 route selection before LLM calls. When a typed runtime registry is active, route selection now only considers `opencode-go` + `deepseek-v4-flash`, rejects typed runtime without explicit routes, and refuses configurations with no v1 route before sampling. Added regression coverage proving a typed runtime turn does not fail over from the v1 route to an `openrouter` route, while ordinary non-runtime provider failover still works. Validation passed: `cargo test -p oxide-agent-core select_model_route_index_does_not_fail_over_typed_runtime_to_non_v1_route -- --nocapture`, `cargo test -p oxide-agent-core run_fails_over_to_weighted_backup_after_persistent_rate_limits -- --nocapture`, `cargo check -p oxide-agent-core`, `cargo fmt --all --check`, `cargo clippy -p oxide-agent-core --all-targets --all-features`. Next: run the broad final validation suite and document any remaining PRD gaps before goal completion.
- 2026-05-22 22:19: Phase 8g fixed final web-transport validation by porting `write_todos` into the typed runtime registry. The v1 registry now shares the same `todos_arc` used by runner memory sync, exposes `write_todos` beside sandbox/SSH tools, and the web scripted parallel-tool E2E now runs on the `opencode-go` + `deepseek-v4-flash` route instead of the removed legacy scripted route. Validation passed: `cargo test -p oxide-agent-core typed_runtime_registry_exposes_sandbox_tools -- --nocapture`, `cargo test -p oxide-agent-core current_tool_definitions_use_typed_runtime_specs_for_v1_route -- --nocapture`, `cargo test -p oxide-agent-core test_todos_write -- --nocapture`, `cargo test -p oxide-agent-transport-web e2e_parallel_tool_execution_latency -- --nocapture`, `cargo test -p oxide-agent-core`, `cargo test -p oxide-agent-transport-web`, `cargo check --workspace`, `cargo fmt --all --check`, `cargo clippy --workspace --all-targets --all-features`. Remaining external validation gap: live OpenCode Go / DeepSeek V4 Flash smoke was not run because no `OPENCODE_GO_API_KEY` is available in the workspace.
...
- Hermetic validation passed:
  - `cargo test -p oxide-agent-core tool_runtime -- --nocapture`
  - `cargo test -p oxide-agent-core opencode_go -- --nocapture`
  - `cargo test -p oxide-agent-core`
  - `cargo test -p oxide-agent-transport-web`
  - `cargo check --workspace`
  - `cargo fmt --all --check`
  - `cargo clippy --workspace --all-targets --all-features`
- Live provider validation not run: requires `OPENCODE_GO_API_KEY` and network access. No secrets were introduced.
