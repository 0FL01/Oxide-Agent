...
Last updated: 2026-05-28 10:25 +03
...
- G17: Safe Markdown rendering
  ...
  - Status: in_progress
  - Evidence collected: Added `crates/oxide-agent-web-ui/src/markdown.rs` with one `MarkdownContent` component and pure `render_markdown`/`sanitize_html` functions using `comrak` with no default features plus `ammonia`. The sanitizer strips script content/event attributes, removes unsafe link protocols, removes image tags, and keeps tables/code blocks. Task transcript user/final-answer content now uses `MarkdownContent`. Code blocks now render copy buttons handled in Rust/WASM through delegated DOM events; `cargo test -p oxide-agent-web-ui markdown` passes with 4 tests including copy-button output. Full task-list rendering, same-origin image policy, and manual rendering QA remain pending.
...
- Backend/socket coverage is now green for the current checkpoint, including the full `profile-lite,socket_e2e` suite and explicit delegation/compression feature checks. The largest remaining PRD gaps are browser/manual QA and remaining Markdown image/task-list policy details.
