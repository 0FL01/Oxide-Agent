...
fn test_function_call_id_roundtrip() {
    let function_call =
        FunctionCall::with_id("test_function", json!({"param": "value"}), "call_123");
...
#[test]
fn test_function_response_id_roundtrip() {
    let function_response =
        FunctionResponse::with_id("test_function", json!({"ok": true}), "call_123");
    ...
    assert_eq!(deserialized, function_response);
}

#[test]
fn test_function_calling_config_allowed_names_serialization() {
    ...
    assert_eq!(serialized["mode"], "ANY");
    assert_eq!(
        serialized["allowedFunctionNames"],
        json!(["get_weather", "get_time"])
    );
