...
История чата и память агента хранятся отдельно в R2:

### Память чата
- **Ключ**: `users/{user_id}/history.json`
- **Очистка**: "Очистить контекст" в главном меню
- **Содержит**: сообщения обычного чата с LLM

### Память агента
- **Ключ**: `users/{user_id}/agent_memory.json`
- **Очистка**: "🗑 Очистить память" в режиме агента
- **Содержит**: контекст агента с автосжатием, персистируется после каждой задачи
- **Загружается** автоматически при активации режима агента

### Полная очистка
- **Кнопка**: "🗑 Очистить всё" в главном меню
- **Очищает**: и историю чата, и память агента
