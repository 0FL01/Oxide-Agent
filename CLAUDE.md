# Проект: Another Chat with LLM (Порт на Rust)
...
- `AGENT.md`: Системный промпт для режима Агента.
...
- **Информация о зависимостях**: Используйте `cargo-info [crate]` для получения подробностей. Избегайте ручного чтения `Cargo.toml` manually.
- **Объяснение ошибок**: Если компилятор выдает код ошибки (например, E0308), **всегда** запускайте `rustc-explain [code]` перед попыткой исправить её.
...
### 3. Документация (КРИТИЧЕСКИ ВАЖНО)
**Хватит гадать API.** Если вы не уверены в использовании крейта:
1.  **Найти крейт**: Используйте `search_crate`, чтобы найти правильное имя пакета.
2.  **Поиск элементов**: Используйте `search_documentation_items` для поиска конкретных структур/функций (нечеткий поиск).
3.  **Чтение документации**: Используйте `retrieve_documentation_page` или `retrieve_documentation_index_page`.
4.  **Запасной вариант**: Используйте `retrieve_documentation_all_items` только если нечеткий поиск не дал результатов.
...
| "Как использовать Vec" | `search_documentation_items` -> `retrieve_documentation_page` |