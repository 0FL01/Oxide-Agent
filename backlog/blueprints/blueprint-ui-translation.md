...
**Steps**:
1. [x] **Implementation**: In `src/bot/views/agent.rs`:
...
2. [x] **Keyboards**: Update keyboard text definitions:
...
3. [x] **Verification**: Run `cargo-check` to ensure string literal syntax is correct.

## Phase 2: Agent Logic Synchronization [x]
...
**Steps**:
1. [x] **Sync Handlers**: In `src/bot/agent_handlers.rs`, update `handle_agent_message` match arms:
    - Match "❌ Cancel Task" instead of Russian equivalent.
    - Match "🗑 Clear Memory".
    - Match "🔄 Recreate Container".
    - Match "⬅️ Exit Agent Mode".
2. [x] **Sync Confirmation**: Update `handle_agent_wipe_confirmation` to match "✅ Yes" and "❌ Cancel".
3. [x] **Dynamic Messages**: Translate internal status messages:
...
4. [x] **QA**: Run `cargo-check` to verify code compiles.

## Phase 3: Main Bot Logic & Menus [x]
...
**Steps**:
1. [x] **Commands**: Update `BotCommands` derive macros to use English descriptions for `/start`, `/clear`, `/healthcheck`, `/stats`.
2. [x] **Keyboards**: Update functions to return English buttons:
...
3. [x] **Sync Handlers**: In `handle_menu_commands`, update `match text` to correspond EXACTLY to the new button texts above.
4. [x] **Responses**: Translate:
...
5. [x] **QA**: Run `cargo-check`.

## Phase 4: Narrator Localization [x]
...
**Steps**:
1. [x] **System Prompt**: In `Narrator::system_prompt()`:
...
2. [x] **Final Verification**: Run `cargo-check` to ensure the project compiles successfully with all changes.
