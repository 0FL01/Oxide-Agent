...
### 🤖 LLM Провайдеры (настраиваются через `.env`)
*   **OpenRouter** (`OPENROUTER_API_KEY`) — чаще всего отвечает за чат и мультимодальные задачи (например, `google/gemini-3-flash-preview`). Убедитесь, что `CHAT_MODEL_PROVIDER=openrouter`, если нужны голосовая/визуальная обработка.
*   **ZAI** (`ZAI_API_KEY`) — основной провайдер для режима агента (`glm-4.7` или `glm-4.5-air`). Хостинг [Zhipu AI](https://z.ai/) обеспечивает поддержку инструментов и структурированные ответы.
*   **Mistral** (`MISTRAL_API_KEY`) — экономичный провайдер для комбинаций агент/чат (например, `devstral-2512`, `mistral-large-latest`).
*   **Groq** (`GROQ_API_KEY`) — опционален, можно использовать `AGENT_MODEL_ID=groq-trooper-1`, `AGENT_MODEL_PROVIDER=groq`.
