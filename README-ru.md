...
Бот разработан с использованием **Rust 1.94**, библиотеки `teloxide`, и поддерживает 6 основных провайдеров для режима чата/агента (**ZAI**, **MiniMax**, **OpenCode Go**, **NVIDIA NIM**, **OpenRouter**, **Mistral**), а также Groq.
</details>
...
<img width="974" height="747" alt="Agent Mode" src="..." />
    ...
    *   **Детерминированный контекст:** Topic `AGENTS.md`, wiki memory, runtime context и включённые инструменты формируют системный промпт без skills RAG.
...
<img width="977" height="762" alt="Video Processing" src="..." />
    ...
    *   **🔗 Система хуков (Hooks):** Расширяемая архитектура для перехвата и кастомизации поведения агента:
        ...
        - Tool Access Policy — применяет allowlist/blocklist инструментов из профиля
...
*   **Поддержка 6 провайдеров для чата/агента:** **ZAI**, **MiniMax**, **OpenCode Go**, **NVIDIA NIM**, **OpenRouter** и **Mistral**. Groq поддерживается **только в режиме чата**.
*   **Нативный Tool Calling:** Эффективное использование инструментов в современных моделях.
*   **Обработка мультимедиа:**
    *   Голосовые и видео сообщения (распознавание речи через Gemini-family модели в OpenRouter или Voxtral).
...
| **Cloudflare R2** | `OXIDE_R2_*` | S3-хранилище (Access Key, Secret, Endpoint, Bucket) |
...
Бот поддерживает 6 основных провайдеров для работы как в обычном режиме чата, так и в продвинутом режиме Агента (с вызовом инструментов):
...
OXIDE_R2_REGION=auto           # Регион S3-совместимого хранилища
...
Повторите схему `_MODEL_ID/_MODEL_PROVIDER` для Groq, Gemini-family моделей через OpenRouter или любых провайдеров, которые вы хотите включить. Меню выбора модели покажет только определенные вами имена.

## Архитектура агента


<details>
<summary>🏗 Внутренняя структура, Контекст, Хуки</summary>

### Детерминированный контекст

Legacy-подсистема skills RAG удалена. Контекст для промпта собирается из явных источников:

- Topic-scoped `AGENTS.md`
- S3/R2-backed wiki memory
- Runtime context injections
- Включённые инструменты и profile instructions
...
- **Delegation Provider** (`delegation.rs`) — асинхронный запуск, ожидание и отмена под-агентов для декомпозиции сложных задач
...
```text
crates/
├── oxide-agent-core/           # Доменная логика, интеграции LLM, хуки, storage
│   └── src/
│       ├── agent/              # Ядро агента и логика выполнения
│       │   ├── hooks/          # Хуки выполнения (Completion, Search, Timeout, Safety)
...
| **OR Gemini 2.5 Flash Lite** | OpenRouter | Дешево и эффективно |
