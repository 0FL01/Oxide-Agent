...
Бот разработан с использованием **Rust 1.94**, библиотеки `teloxide`, и поддерживает 7 основных провайдеров для режима чата/агента (**ZAI**, **MiniMax**, **OpenCode Go**, **NVIDIA NIM**, **OpenRouter**, **Mistral**, **Google Gemini**), а также Groq.
</details>
...
<img width="977" height="762" alt="Video Processing" src="..." />
    ...
    *   **Веб-поиск и извлечение данных:** SearXNG и Tavily используются для поиска, локальный `web_markdown` читает конкретный URL как Markdown.
...
*   **Поддержка 7 провайдеров для чата/агента:** **ZAI**, **MiniMax**, **OpenCode Go**, **NVIDIA NIM**, **OpenRouter**, **Mistral** и **Google Gemini**. Groq поддерживается **только в режиме чата**.
...
| :--- | :--- | :--- |
| **ZAI** | `ZAI_API_KEY` | Требуется, если настроены маршруты ZAI (`glm-4.7` в примерах по умолчанию). [Zhipu AI](https://z.ai/) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` | Требуется для маршрутов `opencode-go`, например `deepseek-v4-flash`. |
...
Бот поддерживает 7 основных провайдеров для работы как в обычном режиме чата, так и в продвинутом режиме Агента (с вызовом инструментов):
...
*   **MiniMax** (`MINIMAX_API_KEY`) — Claude SDK-compatible провайдер через MiniMax API (`MiniMax-M2.7`).
...
*   **NVIDIA NIM** (`NVIDIA_API_KEY`) — hosted NIM маршруты для совместимых tool-calling моделей.
...
#### Другие провайдеры
*   **Groq** (`GROQ_API_KEY`) — быстрый провайдер для специализированных текстовых задач (например, `llama-3.3-70b-versatile`), используется в режиме чата.
...
*   **Local Web Markdown** — лёгкое чтение одного URL через HTTP GET с HTML-to-Markdown и лимитами ответа/вывода
...
SEARCH_PROVIDER=tavily          # Устарело: используйте TAVILY_ENABLED / SEARXNG_ENABLED
...
- **WebFetch Markdown Provider** (`webfetch_md.rs`) — чтение одного URL как Markdown с защитой от больших ответов
...
│       │   ├── providers/      # Провайдеры инструментов (Sandbox, Tavily, WebFetch, Delegation и т.д.)
