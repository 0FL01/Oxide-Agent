# PRD: Deep Refactor Program for `Oxide-Agent` (`archon-ref`)
...
1. **Документация расходится с кодом.** До начала migration нужно решить, где источник истины: для рефакторинга — только код и characterization suite. ([GitHub][9]) - Код - является истинной
2. **Approval flow уже в переходном состоянии.** Его нельзя оставлять “полуживым”, иначе state machine получится ложной. ([GitHub][13]) - выпилить из кода полностью
